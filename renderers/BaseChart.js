...
export class BaseChart {
  ...
  getPersistentChartId(canvas) {
    ...
    const persistentId = 'chart_' + Date.now() + '_' + Math.random().toString(36).slice(2, 11);
    ...
    if (tooltipContent && typeof tooltipContent === 'object' && tooltipContent.items) {
      ...
      tooltipContent.items.forEach(item => {
        ...
        if (item.color) {
          ...
          colorSpot.style.cssText = `width: 8px; height: 8px; background: ${item.color}; border-radius: 50%; flex-shrink: 0;`;
          itemDiv.appendChild(colorSpot);
        }
